            ...
            children: [
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: () {
                    final link = order.trackingToken.isNotEmpty
                        ? 'https://karmencarla.onrender.com/track/?order=${Uri.encodeComponent(order.orderId)}&token=${Uri.encodeComponent(order.trackingToken)}'
                        : 'https://karmencarla.onrender.com/track/?order=${Uri.encodeComponent(order.orderId)}';
                  ...
                  label: const Text('متابعة الطلبية ونسخ رابط التتبع'),
                ),
              ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                '× ${line.quantity}${line.size.isEmpty ? ' • المقاس غير محفوظ لهذا الطلب القديم' : ' • مقاس ${line.size}'}${line.color.isEmpty ? '' : ' • ${line.color}'}',
                style: Theme.of(context).textTheme.bodySmall,
              ),
            ],
          ),
...
class _ProfessionalAmbassadorsScreenState
  ...
  bool _loading = true;
  Object? _error;
  AmbassadorProgramSummary? _program;
  String _query = '';
  String _filter = 'all';
  String _sort = 'approved';
  ...
  Future<void> _load() async {
    ...
    try {
      final result = await AdminApi.instance.ambassadorFinanceSummary();
      if (!mounted) return;
      setState(() {
        _program = result;
      });
  ...
  Widget build(BuildContext context) {
    ...
    if (program == null) {
      return _ProfessionalError(
        error: 'تعذر تحميل مركز المندوبات',
        retry: _load,
      );
    }
    final visible = program.items
        .where(
          (profile) =>
              ...
              switch (_filter) {
                'attention' => profile.needsAttention,
                'active' => profile.status == 'active',
                'balance' => profile.availableBalance > 0,
                'pipeline' => profile.pipelineCommission > 0,
                _ => true,
              },
        )
        .toList();
    visible.sort((a, b) {
      final compare = switch (_sort) {
        'sales' => b.sales.compareTo(a.sales),
        'orders' => b.ordersCount.compareTo(a.ordersCount),
        'pending' => b.pendingApprovalCommission.compareTo(
          a.pendingApprovalCommission,
        ),
        'latest' => b.lastOrderMs.compareTo(a.lastOrderMs),
        _ => b.approvedCommission.compareTo(a.approvedCommission),
      };
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          _AmbassadorFinanceHero(summary: program),
          const SizedBox(height: 12),
          TextField(
            onChanged: (value) => setState(() => _query = value),
            decoration: const InputDecoration(
              prefixIcon: Icon(Icons.search),
              hintText: 'بحث باسم أو هاتف أو بريد أو UID',
            ),
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(
                child: DropdownButtonFormField<String>(
                  initialValue: _sort,
                  decoration: const InputDecoration(
                    labelText: 'ترتيب البطاقات',
                    prefixIcon: Icon(Icons.sort_rounded),
                  ),
                  items: const [
                    DropdownMenuItem(
                      value: 'approved',
                      child: Text('حسب الرصيد المعتمد'),
                    ),
                    DropdownMenuItem(
                      value: 'sales',
                    ...
                    DropdownMenuItem(
                      value: 'orders',
                      child: Text('حسب عدد الطلبات'),
                    ),
                    DropdownMenuItem(
                      value: 'pending',
                      child: Text('حسب العمولات المعلقة'),
                    ),
                    DropdownMenuItem(
                      value: 'latest',
            ...
            child: Row(
              children: [
                for (final entry in const {
                  ...
                  'active': 'النشطات',
                  'balance': 'لديهن رصيد قابل للسحب',
                  'pipeline': 'طلبات جارية',
                }.entries)
                  Padding(
                    padding: const EdgeInsetsDirectional.only(end: 8),
                    child: FilterChip(
          ...
          GridView.count(
            ...
            children: [
              _SummaryTile(
                ...
                title: 'المندوبات',
                value: '${program.ambassadorCount}',
              ),
              _SummaryTile(
                icon: Icons.payments_outlined,
                title: 'إجمالي المبيعات',
                value: '${program.sales.toStringAsFixed(2)} د.ل',
              ),
              _SummaryTile(
                icon: Icons.verified_outlined,
                title: 'عمولات معتمدة',
                value: '${program.approvedCommission.toStringAsFixed(2)} د.ل',
              ),
              _SummaryTile(
                icon: Icons.account_balance_wallet_outlined,
                title: 'قابل للسحب',
                value: '${program.availableBalance.toStringAsFixed(2)} د.ل',
              ),
              _SummaryTile(
                icon: Icons.pending_actions_outlined,
                title: 'بانتظار الاعتماد',
                value:
                    '${program.pendingApprovalCommission.toStringAsFixed(2)} د.ل',
              ),
              _SummaryTile(
                icon: Icons.local_shipping_outlined,
                title: 'عمولات جارية',
                value: '${program.pipelineCommission.toStringAsFixed(2)} د.ل',
              ),
            ],
          ),
          const SizedBox(height: 10),
            ...
            _FinanceAmbassadorCard(
              ...
              onTap: () async {
                await Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) =>
                        AmbassadorDetailsScreen(profileKey: visible[index].key),
                  ),
                );
                if (mounted) _load();
              },
...
class _AmbassadorFinanceHero extends StatelessWidget {
  const _AmbassadorFinanceHero({required this.summary});

  final AmbassadorProgramSummary summary;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: const LinearGradient(
          colors: [Color(0xFF251833), Color(0xFF4A3171)],
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            'كل الأرقام هنا تأتي من سجل عمولات مركزي موحّد بين صفحة المندوبات والمحاسبة والسحب.',
          ...
          Wrap(
            ...
            children: [
              _FinanceHeroChip(
                label: 'عمولات معتمدة',
                value: '${summary.approvedCommission.toStringAsFixed(2)} د.ل',
              ),
              _FinanceHeroChip(
                label: 'قابل للسحب',
                value: '${summary.availableBalance.toStringAsFixed(2)} د.ل',
              ),
              _FinanceHeroChip(
                label: 'طلبات سحب معلقة',
                value:
                    '${summary.pendingWithdrawalsTotal.toStringAsFixed(2)} د.ل',
              ),
            ],
          ),
        ],
...
class _FinanceHeroChip extends StatelessWidget {
  const _FinanceHeroChip({required this.label, required this.value});
...
class _FinanceAmbassadorCard extends StatelessWidget {
  const _FinanceAmbassadorCard({
    required this.rank,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: _MiniValue(
                    label: 'الرصيد',
                    value: profile.availableBalance.toStringAsFixed(2),
                  ),
                ),
              ],
            ),
            ...
            const SizedBox(height: 5),
            Text(
              'نجاح التوصيل ${profile.deliveryRate.toStringAsFixed(1)}% • ${profile.openOrders} طلب مفتوح • ${profile.pendingApprovalCommission.toStringAsFixed(2)} د.ل بانتظار الاعتماد',
              style: Theme.of(context).textTheme.bodySmall,
            ),
            const SizedBox(height: 10),
            Wrap(
              ...
              children: [
                _FinanceBadge(
                  icon: Icons.verified_outlined,
                  label: 'معتمد',
                  value: '${profile.approvedCommission.toStringAsFixed(2)} د.ل',
                  color: Colors.green,
                ),
                _FinanceBadge(
                  icon: Icons.pending_actions_outlined,
                  label: 'معلّق',
                  value:
                      '${profile.pendingApprovalCommission.toStringAsFixed(2)} د.ل',
                  color: Colors.orange,
                ),
                _FinanceBadge(
                  icon: Icons.local_shipping_outlined,
                  label: 'جاري',
                  value: '${profile.pipelineCommission.toStringAsFixed(2)} د.ل',
                  color: Colors.blue,
                ),
              ],
            ),
...
class AmbassadorDetailsScreen extends StatefulWidget {
  const AmbassadorDetailsScreen({super.key, required this.profileKey});

  final String profileKey;

  @override
  State<AmbassadorDetailsScreen> createState() =>
      _AmbassadorDetailsScreenState();
}

class _AmbassadorDetailsScreenState extends State<AmbassadorDetailsScreen> {
  AmbassadorFinanceProfile? _profile;
  bool _loading = true;
  Object? _error;
  String? _busyId;
  ...
  Future<void> _load() async {
    ...
    try {
      final profile = await AdminApi.instance.ambassadorFinanceDetail(
        widget.profileKey,
      );
      if (!mounted) return;
  ...
  Future<void> _updateOrderCommission(AdminOrder order) async {
    final draft = await showModalBottomSheet<_CommissionDraft>(
      context: context,
      isScrollControlled: true,
      showDragHandle: true,
      builder: (_) => _CommissionEditorSheet(
        initialStatus: order.commissionStatus.isEmpty
            ? 'pending'
            : order.commissionStatus,
        initialAmount: order.commissionApprovedAmount > 0
            ? order.commissionApprovedAmount
            : order.commissionBaseAmount,
        initialNote: order.commissionNote,
        title: 'إدارة عمولة الطلب #${order.orderId}',
      ),
    );
    if (draft == null) return;
    setState(() => _busyId = order.orderId);
    try {
      await AdminApi.instance.updateOrderCommission(
        order.orderId,
        status: draft.status,
        approvedAmount: draft.amount,
        note: draft.note,
      );
      await _load();
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('تم تحديث عمولة الطلب')));
      }
    } catch (error) {
      if (mounted) {
        ScaffoldMessenger.of(
  ...
  Future<void> _createManualCommission(bool negative) async {
    final profile = _profile;
    if (profile == null) return;
    final draft = await showModalBottomSheet<_ManualCommissionDraft>(
      context: context,
      isScrollControlled: true,
      showDragHandle: true,
      builder: (_) => _ManualCommissionSheet(isDeduction: negative),
    );
    if (draft == null) return;
    setState(() => _busyId = negative ? 'manual-deduct' : 'manual-add');
    try {
      await AdminApi.instance.createManualCommission(
        profile: profile,
        amount: negative ? -draft.amount : draft.amount,
        note: draft.note,
      );
      await _load();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              negative ? 'تم تسجيل الخصم بنجاح' : 'تمت إضافة العمولة اليدوية',
            ),
          ),
        );
      }
  ...
  Future<void> _editManualRecord(AmbassadorCommissionRecord record) async {
    final draft = await showModalBottomSheet<_CommissionDraft>(
      context: context,
      isScrollControlled: true,
      showDragHandle: true,
      builder: (_) => _CommissionEditorSheet(
        ...
        initialAmount: record.approvedAmount.abs(),
        initialNote: record.note,
        title: record.approvedAmount < 0
            ? 'تعديل قيد خصم يدوي'
            : 'تعديل قيد عمولة يدوية',
        allowNegativeHint: record.approvedAmount < 0,
      ),
    );
    if (draft == null) return;
    setState(() => _busyId = record.id);
    try {
      await AdminApi.instance.updateManualCommission(
        record.id,
        status: draft.status,
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null || _profile == null) {
      return Scaffold(
        ...
        body: _ProfessionalError(error: _error, retry: _load),
      );
    }
    final profile = _profile!;
    final orderRecords = profile.orders;
    final manualRecords = profile.records
        .where((record) => record.sourceType == 'manual')
        .toList();
    final pendingOrders = orderRecords
        .where(
          (order) =>
        ...
        .toList();
    final pipelineOrders = orderRecords
        .where(
          (order) =>
              order.status != 'delivered' &&
              !{'canceled', 'returned', 'returning'}.contains(order.status),
        )
        .toList();
    return Scaffold(
      appBar: AppBar(title: const Text('ملف المندوبة')),
          ...
          children: [
                  ...
                  children: [
                    Row(
                      children: [
                        const CircleAvatar(
                          radius: 28,
                          child: Icon(Icons.campaign_outlined, size: 28),
                        ),
                    ...
                    Wrap(
                      ...
                      children: [
                        _FinanceBadge(
                          ...
                          label: 'قابل للسحب',
                          value:
                              '${profile.availableBalance.toStringAsFixed(2)} د.ل',
                          color: Colors.green,
                        ),
                        _FinanceBadge(
                          icon: Icons.pending_actions_outlined,
                        ...
                        _FinanceBadge(
                          ...
                          label: 'طلبات سحب محجوزة',
                          value:
                              '${profile.reservedWithdrawals.toStringAsFixed(2)} د.ل',
                          color: Colors.blue,
                        ),
                      ],
                    ),
            ...
            Row(
              children: [
                Expanded(
                  child: FilledButton.tonalIcon(
                    onPressed: _busyId == 'manual-add'
                        ? null
                        : () => _createManualCommission(false),
                    icon: _busyId == 'manual-add'
                        ? const SizedBox.square(
                            dimension: 18,
                            child: CircularProgressIndicator(strokeWidth: 2),
                          )
                        : const Icon(Icons.add_card_outlined),
                    label: const Text('إضافة عمولة يدوية'),
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: _busyId == 'manual-deduct'
                        ? null
                        : () => _createManualCommission(true),
                    icon: _busyId == 'manual-deduct'
                        ? const SizedBox.square(
                            dimension: 18,
                            child: CircularProgressIndicator(strokeWidth: 2),
                          )
            ...
            const SizedBox(height: 16),
            _SectionHeaderText(
              title: 'طلبات بانتظار اعتماد العمولة',
              subtitle:
                  'الطلبات الموصلة تظهر هنا إلى أن يتم اعتماد العمولة أو تعديلها أو إلغاؤها.',
            ),
            const SizedBox(height: 8),
            if (pendingOrders.isEmpty)
              const _EmptyStateCard(
                text: 'لا توجد عمولات موصلة بانتظار الاعتماد',
              )
            else
              for (final order in pendingOrders)
                _OrderCommissionCard(
                  order: order,
                  busy: _busyId == order.orderId,
                  onManage: () => _updateOrderCommission(order),
                ),
            const SizedBox(height: 16),
            _SectionHeaderText(
              title: 'عمولات جارية من الطلبات المفتوحة',
              subtitle:
                  'هذه ليست قابلة للسحب بعد، لكنها تساعد على توقع الدخل القادم.',
            ),
            const SizedBox(height: 8),
            if (pipelineOrders.isEmpty)
              const _EmptyStateCard(text: 'لا توجد طلبات نشطة حالياً')
            else
              for (final order in pipelineOrders)
                _OrderCommissionCard(order: order, busy: false, onManage: null),
            const SizedBox(height: 16),
            _SectionHeaderText(
              title: 'القيود اليدوية وسجل التعديلات',
              subtitle:
                  'كل إضافة أو خصم أو تعديل يُحفظ في السجل مع الوقت والملاحظة.',
            ),
            const SizedBox(height: 8),
            if (manualRecords.isEmpty)
              const _EmptyStateCard(
                text: 'لا توجد قيود يدوية مسجلة لهذه المندوبة',
              )
            else
              for (final record in manualRecords)
                Card(
                  child: ListTile(
                    ...
                    subtitle: Text(
                      '${record.note.isEmpty ? 'بدون ملاحظة' : record.note}\n${formatDateTime(record.updatedAtMs)}',
                    ),
                    isThreeLine: true,
                    trailing: _busyId == record.id
                        ? const SizedBox.square(
                            dimension: 20,
                            child: CircularProgressIndicator(strokeWidth: 2),
                          )
                        : Column(
                            ...
                            children: [
                              Text(
                                '${record.approvedAmount.toStringAsFixed(2)} د.ل',
                                style: TextStyle(
                                  fontWeight: FontWeight.w900,
                                  color: record.approvedAmount >= 0
                                      ? Colors.green
                              ...
                              TextButton(
                                onPressed: () => _editManualRecord(record),
            ...
            const SizedBox(height: 16),
            _SectionHeaderText(
              title: 'الخط الزمني المالي',
              subtitle:
                  'عرض موحد للطلبات والقيود اليدوية والسحب حسب آخر تحديث.',
            ),
            const SizedBox(height: 8),
            if (profile.history.isEmpty)
              const _EmptyStateCard(text: 'لا يوجد سجل زمني حتى الآن')
            else
              for (final event in profile.history.take(40))
                _AmbassadorHistoryTile(event: event),
...
class _SectionHeaderText extends StatelessWidget {
  const _SectionHeaderText({required this.title, required this.subtitle});
...
class _OrderCommissionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final amount = isDelivered
        ? (order.commissionApprovedAmount > 0
              ? order.commissionApprovedAmount
              : order.commissionBaseAmount)
        : (order.commissionBaseAmount > 0
              ? order.commissionBaseAmount
              : order.commission(defaultPercent: 7, perProductEnabled: true));
    final color = _statusColor(order.commissionStatus);
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text(
          '${order.buyerName} • ${orderStatuses[order.status] ?? order.status}',
        ),
        trailing: busy
            ? const SizedBox.square(
                dimension: 20,
            ...
            children: [
              ...
              Expanded(
                child: _MiniValue(
                  ...
                  value: '${order.commissionBaseAmount.toStringAsFixed(2)} د.ل',
                ),
              ),
              Expanded(
                child: _MiniValue(
                  label: 'المعتمدة',
                  value:
                      '${order.commissionApprovedAmount.toStringAsFixed(2)} د.ل',
                ),
              ),
            ],
          ),
          if (order.commissionNote.isNotEmpty) ...[
            const SizedBox(height: 10),
            Container(
              width: double.infinity,
              ...
              child: Text('ملاحظة الإدارة: ${order.commissionNote}'),
            ),
          ],
          if (order.lines.isNotEmpty) ...[
            const SizedBox(height: 10),
          ...
          if (onManage != null) ...[
            ...
            Align(
              ...
              child: FilledButton.tonalIcon(
                ...
                label: const Text('إدارة العمولة'),
              ),
            ),
          ],
        ],
...
class _AmbassadorHistoryTile extends StatelessWidget {
  const _AmbassadorHistoryTile({required this.event});

  final AmbassadorFinanceEvent event;

  @override
  Widget build(BuildContext context) {
    final isNegative = event.amount < 0;
    final color = switch (event.type) {
      'withdrawal' => Colors.blue,
      'manual' => isNegative ? Colors.red : Colors.green,
      'commission_action' => Colors.orange,
      _ => Colors.purple,
    };
    return Card(
      child: ListTile(
        ...
        subtitle: Text(
          '${event.note.isEmpty ? 'بدون ملاحظة' : event.note}\n${formatDateTime(event.createdAtMs)}',
        ),
        isThreeLine: true,
        trailing: Text(
          '${event.amount.toStringAsFixed(2)} د.ل',
...
class _CommissionDraft {
  const _CommissionDraft({
...
class _CommissionEditorSheet extends StatefulWidget {
  const _CommissionEditorSheet({
    required this.initialStatus,
    required this.initialAmount,
    required this.initialNote,
    required this.title,
    this.allowNegativeHint = false,
  });
  ...
  final String title;
  final bool allowNegativeHint;

  @override
  State<_CommissionEditorSheet> createState() => _CommissionEditorSheetState();
}

class _CommissionEditorSheetState extends State<_CommissionEditorSheet> {
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(labelText: 'حالة العمولة'),
              items: const [
                DropdownMenuItem(
                  value: 'pending',
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                helperText: widget.allowNegativeHint
                    ? 'هذه القيمة ستُطبّق بنفس إشارة القيد الحالي.'
                    : null,
                suffixText: 'د.ل',
              ),
            ),
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'مثال: تعديل بسبب اتفاق خاص أو خصم مرتجع',
              ),
            ),
            const SizedBox(height: 16),
            FilledButton.icon(
              onPressed: () {
                ...
                if (amount < 0) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('أدخلي قيمة موجبة في هذا الحقل'),
                    ),
                  );
                  return;
                }
...
class _ManualCommissionDraft {
  const _ManualCommissionDraft({required this.amount, required this.note});

  final double amount;
  final String note;
}

class _ManualCommissionSheet extends StatefulWidget {
  const _ManualCommissionSheet({required this.isDeduction});

  final bool isDeduction;

  @override
  State<_ManualCommissionSheet> createState() => _ManualCommissionSheetState();
}

class _ManualCommissionSheetState extends State<_ManualCommissionSheet> {
    ...
    child: Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Text(
              widget.isDeduction ? 'خصم من رصيد المندوبة' : 'إضافة عمولة يدوية',
              style: Theme.of(
                context,
              ).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900),
            ),
            ...
            TextField(
              ...
              maxLines: 4,
              decoration: InputDecoration(
                labelText: widget.isDeduction ? 'سبب الخصم' : 'ملاحظة الإضافة',
                hintText: widget.isDeduction
                    ? 'مثال: تسوية فرق أو خصم خطأ سابق'
                    : 'مثال: مكافأة إضافية أو تعديل يدوي',
              ),
            ),
            const SizedBox(height: 16),
            FilledButton.icon(
              onPressed: () {
                final amount = double.tryParse(_amount.text.trim()) ?? 0;
                if (amount <= 0 || _note.text.trim().isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('أدخلي المبلغ والملاحظة')),
                  );
                  return;
                }
                Navigator.pop(
                  context,
                  _ManualCommissionDraft(
                    amount: amount,
                    note: _note.text.trim(),
                  ),
                );
...
class _ProfessionalMarketingScreenState
  ...
  final _shippingDefault = TextEditingController();
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final shippingPricing = mapValue(config['shippingPricing']);
      ...
      _shippingDefault.text = '${asDouble(shippingPricing['defaultCost'], 25)}';
      _shippingMode =
          {'darb', 'manual'}.contains(textValue(shippingPricing['mode']))
          ? textValue(shippingPricing['mode'])
          : 'darb';
      _shippingRates = _listFrom(shippingPricing['cityRates']);
      if (mounted)
        setState(() {
          _config = config;
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final shippingPricing = mapValue(_config['shippingPricing']);
      shippingPricing['mode'] = _shippingMode;
      shippingPricing['defaultCost'] =
          double.tryParse(_shippingDefault.text) ?? 25;
      shippingPricing['cityRates'] = _shippingRates;
      _config['shippingPricing'] = shippingPricing;
                ...
                _CampaignPage(
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(
                        labelText: 'طريقة احتساب التوصيل',
                      ),
                      items: const [
                        DropdownMenuItem(
                          value: 'darb',
                          child: Text('درب السبيل ثم الأسعار الاحتياطية'),
                        ),
                        DropdownMenuItem(
                          value: 'manual',
                          child: Text('الأسعار التي أضفتها يدويًا'),
                        ),
                      ],
                      onChanged: (value) =>
                          setState(() => _shippingMode = value ?? 'darb'),
                    ),
                    ...
                    const SizedBox(height: 5),
                    const Text(
                      'حددي السعر الافتراضي وأضيفي مدنًا جديدة. يتم تطبيق الأسعار على الموقع عند الحفظ.',
                    ),
                    const SizedBox(height: 12),
                    TextField(
                      controller: _shippingDefault,
                      keyboardType: const TextInputType.numberWithOptions(
                        decimal: true,
                      ),
                      decoration: const InputDecoration(
                        labelText: 'السعر الافتراضي لباقي المدن (د.ل)',
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        IconButton.filled(
                          tooltip: 'إضافة مدينة',
                          onPressed: () {
                            final city = _shippingCity.text.trim();
                            final cost = double.tryParse(
                              _shippingCost.text.trim(),
                            );
                            if (city.isEmpty || cost == null || cost < 0) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text(
                                    'أدخلي اسم المدينة وسعرًا صحيحًا',
                                  ),
                                ),
                              );
                              return;
                            ...
                            setState(() {
                              _shippingRates.removeWhere(
                                (item) =>
                                    textValue(item['city']).trim() == city,
                              );
                              _shippingRates.add({'city': city, 'cost': cost});
                              _shippingCity.clear();
                              _shippingCost.clear();
                            });
                          },
                    ...
                    if (_shippingRates.isEmpty)
                      const Text('لم تتم إضافة أسعار خاصة للمدن بعد.'),
                    for (var index = 0; index < _shippingRates.length; index++)
                      ListTile(
                        contentPadding: EdgeInsets.zero,
                        leading: const Icon(Icons.location_city_outlined),
                        title: Text(textValue(_shippingRates[index]['city'])),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Text(
                              '${asDouble(_shippingRates[index]['cost']).toStringAsFixed(2)} د.ل',
                            ),
                            IconButton(
                              tooltip: 'حذف',
                              onPressed: () => setState(
