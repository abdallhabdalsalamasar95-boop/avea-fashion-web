...
class AdminApi {
  ...
  Future<AmbassadorProgramSummary> ambassadorFinanceSummary({
    int fromMs = 0,
    int toMs = 0,
  }) async {
    ...
    return AmbassadorProgramSummary.fromJson(result);
  }

  Future<AmbassadorFinanceProfile> ambassadorFinanceDetail(String key) async {
    final result = await get('/admin/ambassadors/detail', query: {'key': key});
    if (result['item'] is! Map) {
      throw const AdminApiException('الخادم لم يرجع ملف المندوبة');
    }
    return AmbassadorFinanceProfile.fromJson(
      Map<String, dynamic>.from(result['item'] as Map),
    );
  }

  Future<AmbassadorCommissionRecord> updateOrderCommission(
    String orderId, {
    ...
    final result = await send(
      'PUT',
      '/admin/ambassador-commissions/order/$orderId',
      {'status': status, 'approvedAmount': ?approvedAmount, 'note': note},
    );
    if (result['item'] is! Map) {
      throw const AdminApiException('تعذر تحديث عمولة الطلب');
    }
    return AmbassadorCommissionRecord.fromJson(
      Map<String, dynamic>.from(result['item'] as Map),
    );
  }

  Future<AmbassadorCommissionRecord> createManualCommission({
    required AmbassadorFinanceProfile profile,
    required double amount,
    required String note,
    String status = 'approved',
  }) async {
    final result = await send('POST', '/admin/ambassador-commissions/manual', {
      'ambassadorKey': profile.key,
      'ambassadorUid': profile.uid,
      'ambassadorName': profile.name,
      'ambassadorEmail': profile.email,
      'ambassadorPhone': profile.phone,
      'amount': amount,
      'note': note,
      'status': status,
    });
    if (result['item'] is! Map) {
      throw const AdminApiException('تعذر إنشاء القيد المالي');
    }
    return AmbassadorCommissionRecord.fromJson(
      Map<String, dynamic>.from(result['item'] as Map),
    );
  ...
  Future<AmbassadorCommissionRecord> updateManualCommission(
    String recordId, {
    String? status,
    double? approvedAmount,
    String? note,
    ...
    final result = await send(
      'PUT',
      '/admin/ambassador-commissions/$recordId',
      {'status': ?status, 'approvedAmount': ?approvedAmount, 'note': ?note},
    );
    if (result['item'] is! Map) {
      throw const AdminApiException('تعذر تحديث القيد المالي');
    }
    return AmbassadorCommissionRecord.fromJson(
      Map<String, dynamic>.from(result['item'] as Map),
    );
