...
function CheckoutPageContent() {
  ...
  const pricing = useMemo(() => {
    ...
    return { subtotal: checkoutTotal, discount, shippingCost: delivery, grandTotal: Math.max(0, checkoutTotal - discount), coupon };
  ...
  const confirmPanel = <div className="checkout-confirm-panel">{error && <p className="form-error">{error}</p>}<div><span><small>إجمالي المنتجات</small><strong>{pricing.grandTotal.toFixed(2)} د.ل</strong></span><button className="primary-button checkout-link" disabled={sending}>{sending ? "جاري الإرسال..." : "تأكيد الطلب"}</button></div><small className="secure-note"><LockKeyhole /> أجرة التوصيل تُدفع منفصلة لشركة التوصيل</small></div>;
