...
import { encodeSharedCart } from "@/lib/ambassador-share";
...
export default function CartPage() {
  ...
  return <div className="container inner-page cart-page">
    ...
    {cart.length === 0 ? <div className="empty-state"><ShoppingBag /><h3>سلتك فارغة</h3><p>اكتشفي تشكيلتنا وأضيفي ما تحبين.</p><Link className="primary-button" href="/#collection">ابدئي التسوق</Link></div>
      : <><div className="cart-layout"><div className="cart-main">{ambassador && <div className="ambassador-cart-banner"><BadgeCheck /><div><strong>حساب المندوبة مفعّل</strong><span>تظهر عمولتك المتوقعة على كل قطعة قبل تأكيد الطلب.</span></div></div>}<div className="cart-lines">{cart.map((item) => {
        const maximum = currentMaximum(item, productsById.get(item.productId));
      ...
      <aside className="order-summary"><div className="summary-heading"><span><ShoppingBag /></span><div><small>ملخص بسيط</small><h2>طلبك</h2></div></div><div><span>عدد القطع</span><strong>{itemCount}</strong></div><div><span>المجموع الفرعي</span><strong>{total.toFixed(2)} د.ل</strong></div><div><span>التوصيل</span><span>يُحدد حسب المدينة</span></div>{ambassador && <div className="commission-summary"><span><CircleDollarSign /> عمولتك المتوقعة</span><strong>{expectedCommission.toFixed(2)} د.ل</strong><small>تُعتمد بعد نجاح التوصيل</small></div>}<hr /><div className="summary-total"><span>الإجمالي</span><strong>{total.toFixed(2)} د.ل</strong></div><div className="share-cart-panel"><BadgeCheck /><div><strong>{ambassador ? "قدّمي لعميلتك تجربة AVEA الخاصة" : "شاركي سلتك كاملة"}</strong><small>{ambassador ? "أرسلي اختياراتك باسمك كشريكة معتمدة، وستضيف العميلة بياناتها بنفسها." : "رابط واحد يفتح جميع اختياراتك بالمقاسات والكميات."}</small></div><AmbassadorShareButton title="اختيارات خاصة من AVEA Fashion" text={ambassador ? `اختيارات خاصة لكِ من شريكة AVEA المعتمدة ${ambassador.ambassadorName}. راجعي السلة وأكملي بيانات التوصيل بسهولة.` : "شاهدي سلة اختياراتي من AVEA Fashion."} label={ambassador ? "إرسال السلة باسمك" : "مشاركة السلة"} buildPath={(token) => `/shared-order/?${token ? `ref=${encodeURIComponent(token)}&` : ""}cart=${encodeSharedCart(cart)}`} /></div><Link className="primary-button checkout-link" href="/checkout/">إتمام الطلب <ArrowLeft /></Link><small className="summary-safe"><ShieldCheck /> الدفع عند الاستلام وبياناتك محفوظة بأمان</small></aside></div></>}
  </div>;
}
