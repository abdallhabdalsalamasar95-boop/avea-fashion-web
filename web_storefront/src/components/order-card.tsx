...
import { ChevronDown, Clock3, MapPin, PackageCheck, PhoneCall, RotateCcw, Share2, Trash2, UserRound } from "lucide-react";
...
export function OrderCard({ orderId, status, createdAt, total, itemCount, items = [], delivery, ambassadorPhone, statusReason, statusReasonImageUrl, customer, footerExtra, onCancel, onReorder, reordering = false, canceling = false, trackingToken = "", compact = false }: OrderCardProps) {
  ...
  const trackingUrl = typeof window !== "undefined"
    ? `${window.location.origin}/track/?order=${encodeURIComponent(orderId)}${trackingToken ? `&token=${encodeURIComponent(trackingToken)}` : ""}`
    : `https://karmencarla.onrender.com/track/?order=${encodeURIComponent(orderId)}${trackingToken ? `&token=${encodeURIComponent(trackingToken)}` : ""}`;
  ...
  return <article className={`order-detail-card tracked-order-card status-${status}${compact ? " ambassador-compact-order" : ""}${expanded ? " expanded" : ""}`}>
    {compact ? <>
      <div className="ambassador-order-summary" role="button" tabIndex={0} aria-label={`${expanded ? "إخفاء" : "عرض"} تفاصيل الطلب ${orderId}`} aria-expanded={expanded} aria-controls={detailsId} onClick={(e) => { if ((e.target as HTMLElement).closest("button, a, input, select")) return; setExpanded((current) => !current); }}>
      ...
      <span className="ambassador-order-summary-meta"><em className={`order-status ${status}`}>{orderStatusLabels[status]}</em><b>{total.toFixed(2)} د.ل</b>{delivery?.referenceCode && <small className="order-summary-shipment"><PackageCheck /> {delivery.referenceCode}</small>}<a className="order-track-link order-track-link-compact" href={trackingUrl} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>فتح التتبع</a><button className="order-share-button order-share-button-compact" type="button" onClick={(event) => { event.stopPropagation(); void shareWithFallback({ title: `تتبع الطلب ${orderId}`, text: "تابعي حالة طلبي من Carmen Karla", url: trackingUrl }); }}><Share2 /> مشاركة</button>{footerExtra}</span>
      <ChevronDown aria-hidden="true" className="ambassador-order-expand-icon" />
      </div>
      <div className="compact-order-evidence"><span>{delivery?.courierPhone ? <><b dir="ltr">{delivery.courierPhone}</b><a dir="ltr" href={`tel:${delivery.courierPhone}`}><PhoneCall /> اتصال</a></> : <small>مندوب التوصيل غير معيّن</small>}</span>{(status === "postponed" || status === "canceled") && (statusReason || delivery?.lastError || statusReasonImageUrl) && <span className="compact-order-reason">{statusReason || delivery?.lastError}{statusReasonImageUrl && <a href={statusReasonImageUrl} target="_blank" rel="noreferrer"><img src={statusReasonImageUrl} alt="صورة السبب" /></a>}</span>}</div>
    </> : <header className="saved-order-main order-detail-card-head">
    ...
    {expanded && <div className={compact ? "ambassador-order-expanded" : ""} id={detailsId}>
      ...
      {detailItems.length > 0 && <div className="order-product-lines">
        {detailItems.map((item, index) => <div className="order-product-line" key={`${item.productId ?? "item"}-${index}`}>
          ...
          <div className="order-product-copy"><strong>{item.name || "موديل بدون اسم"}</strong>{item.productCode && <small>كود: {item.productCode}</small>}<span>{[item.size && `المقاس ${item.size}`, item.length && `الطول ${item.length}`, item.color && `اللون ${item.color}`].filter(Boolean).join(" • ") || "المقاس غير محفوظ لهذا الطلب القديم"}</span></div>
      ...
      <footer className="order-card-footer"><span>{itemCount} قطعة</span><span>قيمة الطلب <strong>{total.toFixed(2)} د.ل</strong></span>{!compact && footerExtra}<a className="order-track-link" href={trackingUrl} target="_blank" rel="noreferrer"><PackageCheck /> فتح رابط التتبع</a><button className="order-share-button" type="button" onClick={() => void shareWithFallback({ title: `تتبع الطلب ${orderId}`, text: "تابعي حالة طلبي من Carmen Karla", url: trackingUrl })}><Share2 /> مشاركة الرابط</button>{onReorder && status === "canceled" && <button className="order-reorder-button" type="button" disabled={reordering} onClick={onReorder}><RotateCcw /> {reordering ? "جاري الإضافة..." : "إعادة الطلب"}</button>}{onCancel && <button className="order-cancel-button" type="button" disabled={canceling} onClick={onCancel}><Trash2 /> {canceling ? "جاري الإلغاء..." : "إلغاء الطلب"}</button>}</footer>
    </div>}
  </article>;
}