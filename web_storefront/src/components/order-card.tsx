...
export function OrderCard({ orderId, status, createdAt, total, itemCount, items = [], delivery, ambassadorPhone, statusReason, statusReasonImageUrl, customer, footerExtra, onCancel, onReorder, reordering = false, canceling = false, trackingToken = "", compact = false }: OrderCardProps) {
  ...
  return <article className={`order-detail-card tracked-order-card status-${status}${compact ? " ambassador-compact-order" : ""}${expanded ? " expanded" : ""}`}>
    {compact ? <>
      ...
      <span className="ambassador-order-summary-meta"><em className={`order-status ${status}`}>{orderStatusLabels[status]}</em><b>{total.toFixed(2)} د.ل</b>{delivery?.referenceCode && <small className="order-summary-shipment"><PackageCheck /> {delivery.referenceCode}</small>}<button className="order-share-button order-share-button-compact" type="button" onClick={(event) => { event.stopPropagation(); void shareWithFallback({ title: `تتبع الطلب ${orderId}`, text: "تابعي حالة طلبك من Carmen Karla:", url: trackingUrl }); }}><Share2 /> مشاركة التتبع</button>{footerExtra}</span>
    ...
    {expanded && <div className={compact ? "ambassador-order-expanded" : ""} id={detailsId}>
      ...
      <footer className="order-card-footer"><span>{itemCount} قطعة</span><span>قيمة الطلب <strong>{total.toFixed(2)} د.ل</strong></span>{!compact && footerExtra}<button className="order-share-button" type="button" onClick={() => void shareWithFallback({ title: `تتبع الطلب ${orderId}`, text: "تابعي حالة طلبك من Carmen Karla:", url: trackingUrl })}><Share2 /> مشاركة التتبع</button>{onReorder && status === "canceled" && <button className="order-reorder-button" type="button" disabled={reordering} onClick={onReorder}><RotateCcw /> {reordering ? "جاري الإضافة..." : "إعادة الطلب"}</button>}{onCancel && <button className="order-cancel-button" type="button" disabled={canceling} onClick={onCancel}><Trash2 /> {canceling ? "جاري الإلغاء..." : "إلغاء الطلب"}</button>}</footer>
    </div>}
  </article>;
}